...
float calculateDistance(Vector3 p1, Vector3 p2) {
  Vector3 diff = p1 - p2;
  float distance = sqrt(diff.getX() * diff.getX() + diff.getY() * diff.getY());   
  return distance;
}
...
pair<Float, Float> Scene::getApPe(unsigned int object) {
  ...
  for(ii = begin; ii != end; ii++) {
    ...
    mass = ii->getMass();
    force = mass / (calculateDistance(ownPos, pos) * calculateDistance(ownPos, pos));
    Vector3  vrr = ii->getPosition();
    if(force > maxForce) {
      maxForce = force;
      centerMass = ii;
    }
  ...
  return(make_pair<Float, Float>(m_objects[1].getDPosition().getX(), m_objects[1].getDPosition().getY()));
}
